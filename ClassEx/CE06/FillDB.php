...
//insert
    ...
    foreach ($planets as $Newplanets) {
        $insert = "INSERT INTO `CSIS2440`.`Planets` (`PlanetName`, `PosX`, `PosY`, `PosZ`, `Desc`) "."VALUES ('".$Newplanets['name']."', '".$Newplanets['x']."', '".$Newplanets['y']."', '".$Newplanets['z']."', '".$Newplanets['description']."')";
...
//update
    ...
    echo "<br><h2>Making some visitable</h2><br><hr>";
    for ($i = 0; $i < sizeof($planets); $i+=2) {
        ...
        if ($success == FALSE) {
            ...
            echo $planets[$i]['name']." was given a station<br>";
        }
    }
...
//search
    ...
    while ($row = $return->fetch_assoc()){
        echo "<tr><td>".$row['PlanetName']."</td><td style='text-align:center;'>".$row['PosX'].", ".$row['PosY'].", ".$row['PosZ']."</td><td>".$row['Desc']."</td><td>".$row['Active']."</td></tr>";
    }
    echo '</table>';
    mysqli_close($con);
