...
<html>
    ...
    <div class="container-fluid">
                            ...
                            <select id="Select_Product" name="Select_Product" class="select">
                                ...
                                while ($row = mysqli_fetch_array($return)) {
                                    if ($row['idProducts'] == $product_id){
                                        echo "<option value='" . $row['idProducts'] . "' selected='selected'>" . $row['Name'] . "</option>\n";
                                    } else {
                                        echo "<option value='" . $row['idProducts'] . "'>" . $row['Name'] . "</option>\n";
                                    }
                                }
                                ?>
                            </select></p>
        ...
        <div id="Display_cart" class="col-sm-8">
                    <?php
                    if ($_SESSION['cart']){
                        echo "<table border=\"1\" padding=\"3\" width=\"750px\"><tr><th>Name</th><th>Quantity</th><th>Price</th><th>"
                        . "Line Cost</th></tr>";
                        foreach($_SESSION['cart'] as $product_id => $quantity){
                            $sql = "SELECT Name,Price FROM CSIS2440.Products WHERE idProducts =" .$product_id;
                            ...
                            if (mysqli_num_rows($result) > 0){
                                ...
                                echo "<td align=\"center\ width=\"150px\">". money_format('%(#8n', $line_cost) . "</td>";
                                echo "</tr>";
                            }
                        }
                        echo "<tr>";
